...
class WaveVisualizer:
    ...
    def update(self, wave_sim):
        self.field = torch.tensor(wave_sim.get_field(), device=self.device)

        if self.intensity is None:
            self.intensity = torch.zeros_like(self.field)
    ...
    def render_intensity(self, brightness_scale=1.0, exp=0.5):
        # Assuming self.intensity is a 2D tensor representing the intensity of each pixel
        # Normalize intensity values and convert to indices
        normalized_intensity = torch.clip((self.intensity ** exp) * brightness_scale, 0, 1)
        gray_indices = (normalized_intensity * 255).to(torch.int64)

        # Flatten gray_indices for direct indexing
        flat_gray_indices = gray_indices.flatten()

        # Ensure that the indices are on the same device as the colormap
        # This step is crucial to avoid device mismatch errors
        flat_gray_indices = flat_gray_indices.to(self.intensity_colormap.device)

        # Directly index into the colormap tensor
        flat_img = self.intensity_colormap[flat_gray_indices]

        # Reshape the flat image back to the original 2D shape with color channels
        H, W = gray_indices.shape
        img = flat_img.view(H, W, -1)  # Assuming your colormap produces 3 (RGB) or 4 (RGBA) values

        # Convert to a NumPy array for visualization if necessary
        # Ensure to move the tensor back to CPU if it's on GPU/MPS
        img_np = img.cpu().numpy()

        return img_np

    def render_field(self, brightness_scale=1.0):
        gray = (torch.clamp(self.field * brightness_scale, -1.0, 1.0) * 127 + 127).to(torch.uint8)

        # Flatten gray to use it for indexing, then reshape the result back to an image.
        flat_gray = gray.flatten().to(torch.long).cpu()
        flat_img = self.field_colormap[flat_gray] if self.field_colormap is not None else gray.cpu()
        img = flat_img.view(gray.shape[0], gray.shape[1], -1)  # Reshape to [1080, 1920, 3 or 4].

        # Convert to a NumPy array for OpenCV, ensuring to move to CPU if necessary.
        img_np = img.cpu().numpy()
...
def get_colormap_lut(name, invert, black_level=0.0):
    if name == 'icefire':
        color_values = np.array(colormap_icefire) / 255
    elif name == 'colormap_wave1':
        color_values = np.array(colormap_wave1) / 255
    elif name == 'colormap_wave2':
        color_values = np.array(colormap_wave2) / 255
    else:
    ...
    color_values = color_values * (1.0 - black_level) + black_level

    return torch.tensor((color_values * 255).astype(np.uint8), device=device)  # Keep LUT on CPU for OpenCV compatibility
